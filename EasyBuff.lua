		...
		for unitName,auraGroupKeys in pairs(EasyBuff.UnitBuffQueue) do
			...
			if (auraGroupKeys ~= nil and auraGroupKeys ~= {} and auraGroupKeys[1] ~= nil) then
				...
				if (spellId ~= nil) then
					local spellInfo = {GetSpellInfo(spellId)};
					if (spellInfo ~= nil) then
						EasyBuff:AnnounceBuff(format(L["%s needs %s"].."|r", EasyBuff:Colorize(unitName, EasyBuff.CLASS_COLORS[select(2,UnitClass(unitName))])..textColor, tostring(spellInfo[1])));
					end
				end
			end
		end
...
function EasyBuff:RebuildBuffQueue()
	...
	if (IsInGroup(LE_PARTY_CATEGORY_HOME) or IsInRaid(LE_PARTY_CATEGORY_HOME)) then
		for groupIndex=1,GetNumGroupMembers() do
			unitName, groupRank, partyIndex, unitLevel, _, unitClass, unitZone, isOnline, isDead, unitRole, unitIsML = GetRaidRosterInfo(groupIndex);
